...
Instance Proper_flip A `{OType A} : Proper (oleq --> oleq) (@flip A).
Proof.
  intros a1 a2 Ra; assumption.
Qed.

Instance Proper_unflip A `{OType A} : Proper (oleq --> oleq) (@unflip A).
Proof.
  intros a1 a2 Ra; assumption.
Qed.

Program Definition flip_ofun {A B} `{OType A} `{OType B}
        (f: A -o> B) : Flip A -o> Flip B :=
  {| ofun_app := fun a => flip (f @@ unflip a) |}.
Next Obligation.
  intros a1 a2 Ra. rewrite Ra. reflexivity.
Defined.

Program Definition unflip_ofun {A B} `{OType A} `{OType B}
        (f: Flip A -o> Flip B) : A -o> B :=
  {| ofun_app := fun a => unflip (f @@ flip a) |}.
Next Obligation.
  intros a1 a2 Ra. rewrite Ra. reflexivity.
Defined.

Lemma flip_unflip {A} `{OType A} (a:Flip A) : flip (unflip a) =o= a.
Proof. destruct a. reflexivity. Qed.

Lemma flip_unflip_ofun {A B} `{OType A} `{OType B} (f: Flip A -o> Flip B) :
  flip_ofun (unflip_ofun f) =o= f.
Proof.
  apply funOExt; intros b. simpl. repeat rewrite flip_unflip. reflexivity.
Qed.

Lemma unflip_flip_ofun {A B} `{OType A} `{OType B} (f: A -o> B) :
  unflip_ofun (flip_ofun f) =o= f.
Proof.
  apply funOExt; intros b. reflexivity.
Qed.
...
Definition unequiv_ofun {A} `{OType A} : Equiv A -o> A :=
  {| ofun_app := unequiv |}.

Program Definition unequiv'_ofun {A} `{OType A} : Equiv A -o> Flip A :=
  {| ofun_app := fun a => flip (unequiv a) |}.
Next Obligation.
  intros a1 a2 [ Ra1 Ra2 ]. assumption.
Defined.
