...
Lemma zip_unzip_PChain {f1 f2} `{PDiagram f1} `{PDiagram f2} :
  zipPChain ∘ unzipPChain =o= id_ofun.
Proof.
Admitted.

Lemma unzip_zip_PChain {f1 f2} `{PDiagram f1} `{PDiagram f2} :
  unzipPChain ∘ zipPChain =o= id_ofun.
Proof.
Admitted.
...
Section productFunctor.
  ...
  Global Program Instance ProductFMap : FMap ProductF :=
    fun _ _ _ _ =>
      {| ofun_app := fun f => pair_ofun (fmap @@ f ∘ fst_ofun)
                                     (fmap @@ f ∘ snd_ofun) |}.
  Next Obligation.
    intros f g Heq1 [x y] [z w] [Heq2 Heq3]; simpl in *;
      split; try rewrite Heq2; try rewrite Hew3; destruct fmap.
    simpl. apply ofun_Proper; auto. reflexivity.
    simpl. destruct fmap. simpl. rewrite Heq3.
    apply ofun_Proper0; auto. reflexivity.
  Qed.
  ...
  (* Proof. *)
(*     constructor. *)
(*     - intros A oA. unfold fmap. *)
(*       unfold ProductFMap. *)
(*       split; simpl. *)
(*       + intros [x y] [z w] Heq; simpl. *)
(*         apply pair_oeq_inv in Heq; destruct Heq as [Heq1 Heq2]. *)
(*         split. *)
(*         * destruct func1. admit. *)
(*         * admit. *)
(*       + admit. *)
(*     - admit. *)
(*   Admitted. *)
...
Section preorderFunctor.
  ...
  (* Lemma Proper_clos_trans {A} `{OType A} {R : relation A} : *)
  ...
  Lemma Proper_clos_trans {A} `{OType A} :
    Proper (oleq ==> oleq) clos_trans.
  Admitted.
  ...
  (* Next Obligation. *)
  (*   intros ? ? ? ? ? ?. *)
  (*   unfold Basics.impl in *. simpl in *. *)
  (*   generalize Proper_clos_trans; intros Hprop. *)
  (*   unfold Proper, respectful in Hprop. *)
  (*   (* assert (fmapProperRel1 (fmap @@ f) (proj1_sig x) <o= *) *)
  (*   (*         fmapProperRel1 (fmap @@ f) (proj1_sig y)). *) *)
  (*   (* { admit. } *) *)
  (* Admitted. *)
  Admit Obligations.
...
Section tree.
  ...
  Proof. intros x y Hleq z; split; auto; reflexivity. Qed.
  ...
  (* Lemma tree_fmap_comp A B `{OType A} `{OType B} : *)
...
(** TODO: need a construction for mapping Tree over a diagram. *)
(* Program Definition treeFold {f} `{PDiagram f} : *)
(*         Tree (PChain f) -o> PChain (fun n => Tree (f n)) := _. *)
